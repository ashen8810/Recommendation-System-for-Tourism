...
data.columns = ["Name", "Url","Location","ratings","Features","Price","Description"]

data.Location = data.Location.str.replace(", Sri Lanka", "")
...
def column4(data):
  ...
  for i in f:
    d.append(i.text)
  return d
data['Features'] = data['Features'].apply(column4)
...
def column5(data):
  ...
  try:
    ...
    f = soup.findAll("div",{"class":"zV27-freebies"})

    for i in f:
      d.append(str(i.text).strip().split("\n"))
  except:
      d.append(["None"])

  return d
data['Description'] = data['Description'].apply(column5)

data = data.drop(columns = ['Url'],axis = 1,inplace=False)
...
guests = []
for i in range(100):
  guests.append(random.randint(a=1,b =5))
...
res = requirementbased('Kandy',2,2100,'i want Air conditioned room and coffee machine wifi')
